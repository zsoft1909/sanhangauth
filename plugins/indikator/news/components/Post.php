...
class Post extends ComponentBase
{
    ...
    public function onRun()
    {
        ...
        $this->page['nextPost'] = $this->next();
        $this->page['prevPost'] = $this->prev();
        $this->page['userPost'] = $post->user;

        $post_other = NewsPost::where('status', 1)->where('id', '!=', $post->id)->where('published_at', '<', date('Y-m-d H:i:s', time()))
        ->take(4)->orderByRaw('published_at - id desc')->get()->all();
        foreach ($post_other as $key => $other) {
            $other->categories->each(function($category) use ($key, $post_other) {
                $post_other[$key]->category = $category;
            });
        }
        $this->page['post_other'] = $post_other;
    ...
    protected function loadPost()
    {
        ...
        $this->page->meta_image = $post->image;
