...
class UserProfile extends ComponentBase
{
	...
	public function onRun()
	{
		...
		$this->addComponentTab('voucher', [
			'title' => 'Ví Voucher',
			'order' => 30,
			'content' => $this->renderPartial('@voucher')
		]);

		$this->addComponentTab('feadback', [
			'title' => 'Gửi ý kiến cho 3MESTORE',
			'order' => 40,
			'content' => $this->renderPartial('@feadback')
		]);
		...
		$this->page->meta_title = 'Thông tin tài khoản';
		$user = $this->page['user'];
		if ($user) {
			$this->page['provinces'] = \EgalHTN\VietnameseAddress\Classes\Province::getList();
			if ($user->cityid) {
				$this->page['districts'] = \EgalHTN\VietnameseAddress\Classes\District::getList($user->cityid);
			}
			if ($user->districtid) {
				$this->page['wards'] = \EgalHTN\VietnameseAddress\Classes\Ward::getList($user->districtid);
			}
		}
	}
	...
	public function onGetDistrict(){
		...
		$province_code = request()->input('cityid');
		$district = \EgalHTN\VietnameseAddress\Classes\District::getList($province_code);
		
		$view_partial = [
			'#district' => $this->renderPartial('@option-select', [
				'dataArray' => $district,
				'default' => '<option value="">Chọn Quận/Huyện</option>'
			])
		];
		if (count($district) == 0) {
			$view_partial['#ward'] = $this->renderPartial('@option-select', [
				'dataArray' => [],
				'default' => '<option value="">Chọn Xã/Phường</option>'
			]);
		}
	...
	public function onGetWard(){

		$district_code = request()->input('districtid');
		$ward = \EgalHTN\VietnameseAddress\Classes\Ward::getList($district_code);

		return [
			'#ward' => $this->renderPartial('@option-select', [
			'dataArray' => $ward,
			'default' => '<option value="">Chọn Xã/Phường</option>'
		])];
	}
	...
	public function onRecovery()
	{
		...
		if (!$user) {
			Flash::error('Email đăng nhập không tồn tại');
			return;
		}
		...
		Flash::success('Mật khẩu của bạn đã được đặt lại, bạn sẽ nhận được email có mật khẩu mới');
		return;
	}
