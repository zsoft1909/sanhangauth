...
class CartButton extends ComponentBase
{
    ...
    public function onSearchTagProduct(){
    	$search = input('search');

    	$tags = [];
    	$query = Item::select(Db::raw('GROUP_CONCAT(tags) as tags'));
		...
		$tagsAll = $query->groupBy('tags')->first();

		if ($tagsAll) {
			$tags = explode(',', $tagsAll->tags);
		}

    	return [
    		'#content-tags' => $this->renderPartial('@tag-search', [
				'tags' => $tags
			])
    	];
    }
