...
class Cart{
	...
	public function addItem($item, $description, $price, $quantity, $index = null, $extraData = array()){
		$tax = 0.00;
		...
		$data = [
			...
			"index" => $index,
			"title" => $item->name,
			"image" => $item->image_default,
			"description" => $description,
			"tax" => $tax,
			"price" => floatval($price),
			"old_price" => floatval($item->old_price),
			"on_sale" => $item->getPercentSale(),
		...
		// if($item->image)
			// $data['thumb'] = $item->image->getPath(); // getThumb(42, 42, 'crop');
		...
		foreach ($this->items as $key => $val) {
			if ($val['id'] == $item->id) {
				$add_quantity = true;
				if ( !empty($index) && $val['index'] != $index) {
					$add_quantity = false;
				}
				if ($add_quantity) {
	...
	public function updateTotals(){
		...
		*/
		...
		if (!empty($this->coupon)) {
			...
			$this->coupon['amount'] = $coupon->getFinalDiscount($total);
		}
	...
	public static function getCartFromOrder($order){
		...
		$cart->customer_name = $order->customer_name;
		$cart->customer_address = $order->customer_address;
