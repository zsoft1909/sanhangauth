...
jQuery(document).ready(function($) {
	...
	$(document).on('click', '.variant-option:not([disabled="disabled"])', function(){
        $(this).parents('.product-variant').find('.variant-option').removeClass('active');
        $(this).addClass('active');

        $(this).parents('.product-variant').find('.shop__product-input').val($(this).data('value')).trigger('change')
        return false
    });

    if ($('.variant-option.active').length > 0) {
    	setTimeout(function(){
    		$('.variant-option.active').trigger('click');
    	}, 300)
    }

	$('.shop__product-input').change(function(event) {
        var total = 0.00;
        var total_with_tax = 0.00;
        var old_total = 0.00;
        var old_total_with_tax = 0.00;
        var max = 9999;
        var options = [];
        var options_index = [];
        ...
        if($('[name="base_price"]').val()){
            total += parseFloat($('[name="base_price"]').val());
            total_with_tax += parseFloat($('[name="base_price_with_tax"]').val());
        }

        if($('[name="old_price"]').val()){
            old_total += parseFloat($('[name="old_price"]').val());
            old_total_with_tax += parseFloat($('[name="old_price_with_tax"]').val());
        }
        
        $('.variant-option.active').each(function(index, el) {
            ...
            if($(this).data('price')){
                ...
                old_total += parseFloat($(this).data('price'));

                total_with_tax += (parseFloat($(this).data('price')) + parseFloat($(this).data('tax')));
                old_total_with_tax += (parseFloat($(this).data('price')) + parseFloat($(this).data('tax')));
            }

            var input = $(this).parents('.product-variant').find('.shop__product-input');
            if(input.val()){
                options.push($.trim(input.data('label')) + ': ' + $.trim($(this).text()));
                options_index.push($.trim(input.data('slug')) + '::' + $.trim(input.val()));
            }

        });

        let currency = $('.box_detail_product').data('currency');
        $('.box_detail_product ins').html(formatMoney(total_with_tax, 0, ',', '.') +  currency);
        $('.box_detail_product del').html(formatMoney(old_total_with_tax, 0, ',', '.') + currency);

        form.find('[name="price"]').val(total);
        $('.product-quantity input[name="quantity"]').attr('max', max);

        if($('.product-quantity input[name="quantity"]').val() > max)
            $('.product-quantity input[name="quantity"]').val(max);

        form.find('[name="options"]').val(options.join(', '));
        form.find('[name="options_index"]').val(options_index.join(','));
