jQuery.extend(jQuery.validator.messages, {
    ...
    maxlength: jQuery.validator.format("Nhập không quá {0} kí tự"),
...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.steps-payment', function(){
        var $this = $(this);
        $('body').append('<div class="popup-backdrop fade in loading"><div class="modal-content popup-loading-indicator"></div></div>');
        setTimeout(function (){
            $('.popup-backdrop').remove();
            $this.parents('.form-bag').hide();
            $('.payment-hide').hide();
            $('#shop__cart-steps').show();
            $('.store-count').each(function(){
                var quantity = $(this).prev().find('.quantity').val();
                $(this).text(quantity);
            })
        }, 1000);
    });

    $(document).on('change', '#shop__cart-partial input.quantity', function(){
        $.request('onUpdateFromCart', {
            data: {
    ...
    $(document).on('change', '.allItem', function(){
        if ($(this).prop('checked')){
            $('.itemIndex, .allItem').prop('checked', true);
        }else{
            $('.itemIndex, .allItem').prop('checked', false);
        }
        refresh_checkbox_item_cart();
    })

    $(document).on('change', '.itemIndex', function(){
        refresh_checkbox_item_cart();
    });

    function refresh_checkbox_item_cart() {
        let checked = 0;
        let total = 0;
        ...
        $('.chose-filter label span').text('(' + checked + ')');
        $('.chose-filter button, .chose-filter label span').hide();
        if ( checked > 0 ) {
            $('.chose-filter button, .chose-filter label span').show();
        }
